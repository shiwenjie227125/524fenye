        ...
        $stateProvider
            .state(i.route,{
                ...
                controller:function ($scope,$stateParams,alldata,pre,cutpage,$filter) {
                    var cutfn=function (data) {
                        $scope.data=data
                        $scope.fileData=function () {
                            return $filter('filter')($scope.data,{role:$scope.roleName,state:$scope.statues,name:$scope.names})
                        }
...
//        定义中间页数显示的长度,只能为奇数；
                    ...
                    switch (i.id){
                        case 22:{
                            //角色管理
                            cutfn(alldata.fourdata)
                        }break;
                        case 23:{
                            //用户管理
                            cutfn(alldata.thirdata)
                        }break;
                    }
